...
class Archiver(Utils):
    ...
    def clear_save_files(self, folder, keepsaves=False):
        ...
        self.show_step_start(f"Clearing save and log files...", nl=True)
        savefiles = glob.glob(f"{folder}/**/**.save", recursive=True)
        logfiles = glob.glob(f"{folder}/log.txt", recursive=True)
        renpyfolder = os.path.expanduser(os.path.join('~', '.renpy'))

        if len(savefiles) == 0 and len(logfiles) == 0 and not os.path.exists(renpyfolder):
            self.show_step_end("No saves or logfiles found...")
            return

        if os.path.exists(renpyfolder):
            self.show_step_start("Clearing save files from %irenpy%R folder", style=2)
            rmtree(renpyfolder)
            sleep(0.3)

        if len(savefiles) > 0:
            for item in savefiles:
                self.show_step_start("Clearing save files from %igame%R folder", style=2)
                os.remove(item)
            sleep(0.3)

        if len(logfiles) > 0:
            for item in logfiles:
                self.show_step_start("Clearing save files from %ilog.txt%R folder", style=2)
                os.remove(item)
            sleep(0.3)
