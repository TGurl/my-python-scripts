...
class Utils:
    ...
    def open_files(self, story, notes):
        overall = os.path.join(Config.storydir, "overall_notes.md")
        cmd = "nvim" if self.nvim else "vim"
        complete_cmd = f"{cmd} -p {story} {notes} {overall}"
    ...
    def create_a_new_story(self):
        ...
        while True:
            ...
            if title == "":
                ...
                filepath = os.path.join(Config.storydir, filename)
                notepath = filepath.replace(".md", "_notes.md")
                overallpath = os.path.join(Config.storydir, "overall_notes.md")

                if os.path.exists(overallpath):
                    overallnotes = True
                if os.path.exists(filepath):
        ...
        if not exit_menu and filepath != "":
            with open(filepath, "w") as story:
                ...
                story.write(
                    "This story is a work of fiction. Any resemblance to any person living or dead is\n"
                )
                story.write(
                    "purely coincidental. All characters are presumed to be of legal age.\n\n"
                )
                story.write("## Chapter One\n\n")

            with open(notepath, "w") as note:
                note.write(f"# {title} notes\n\n")

            if not overallnotes:
                with open(overallpath) as overallnotes:
                    overallnotes.write("# Overall Notes\n\n")
    ...
    def process_choice(self, choice):
        match choice:
            case "n":
                self.create_a_new_story()
            case "c":
                self.continue_last_story()
            case "d":
                self.delete_a_story()
            case "o":
                self.edit_overall_notes()
            case _:
                self.open_story(int(choice))
    ...
    def menu(self):
        ...
        while True:
            ...
            if len(stories) == 0:
                Config.laststory = ""
                self.myprint(
                    "%gYou didn't write any stories yet...%R", italic=True, nl=True
                )
            for count, story in enumerate(stories, start=1):
                valid.append(str(count))
                story_name = (
                    story.split("/")[-1].replace("_", " ").replace(".md", "").title()
                )
            ...
            if len(stories) > 0:
                self.myprint("%c[%yd%c]%R Delete a story")
            self.myprint("%c[%yo%c]%r Add overall notes", nl=True)
